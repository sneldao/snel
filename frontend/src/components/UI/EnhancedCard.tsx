...
import { motion, AnimatePresence, useAnimation, Variants, useDragControls, PanInfo } from 'framer-motion';
import {
  FaChevronDown,
  FaChevronUp,
  FaEllipsisH,
...
const AnimatedCounter: React.FC<{ value: number; duration?: number; formatter?: (value: number) => string }> = ({ 
  value, 
  duration = 1, 
...
const TrendIndicator: React.FC<{ direction: TrendDirection; value: number }> = ({ 
  direction, 
  value 
}) => {
...
const MiniChart: React.FC<{ 
  ...
  const chartData = data.length > 0 ? data : Array.from({ length: 24 }, (_, i) => ({
    time: i,
    value: 50 + Math.random() * 50 * (trendDirection === 'up' ? 1 : trendDirection === 'down' ? -1 : (i % 2 === 0 ? 1 : -1))
  }));
    ...
    <Box height={`${height}px`} width={`${width}px`}>
      <ResponsiveContainer width="100%" height="100%">
        <AreaChart data={chartData} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
          <defs>
            <linearGradient id={`colorGradient-${trendDirection}`} x1="0" y1="0" x2="0" y2="1">
...
const TransactionStatusBadge: React.FC<{ status: TransactionStatus }> = ({ status }) => {
  ...
  const getStatusIcon = () => {
    switch (status) {
      case 'pending': return FaExchangeAlt;
...
const TransactionTypeBadge: React.FC<{ type: TransactionType }> = ({ type }) => {
  const getTypeColor = () => {
    switch (type) {
      ...
      case 'stake': return 'teal';
      case 'unstake': return 'orange';
      case 'approve': return 'gray';
      default: return 'gray';
    }
  };
  ...
  const getTypeIcon = () => {
    switch (type) {
      ...
      case 'swap': return FaExchangeAlt;
      case 'bridge': return FaExchangeAlt;
...
const NotificationIndicator: React.FC<{ type: NotificationType; isRead?: boolean }> = ({ 
  type, 
  isRead = false 
}) => {
  const getTypeColor = () => {
...
const PortfolioTokenCard: React.FC<{ token: TokenInfo }> = ({ token }) => {
  const { symbol, name, icon, balance, value, price, priceChange, chartData } = token;
    ...
    <Box>
      ...
      <Flex justify="space-between" align="flex-end" mb={3}>
        <Box>
          ...
          {value && <Text fontSize="sm" color="gray.500">${value}</Text>}
        </Box>
        
        <Box>
          <Text fontSize="xs" color="gray.500" textAlign="right">Price</Text>
          <Text fontWeight="bold" textAlign="right">${price}</Text>
        </Box>
      </Flex>
...
// Transaction Card
const TransactionCard: React.FC<{ transaction: TransactionInfo }> = ({ transaction }) => {
  const { 
    hash, 
    type, 
    status, 
  ...
  return (
    <Box>
      ...
      <Flex justify="space-between" mb={4}>
        <Text fontSize="sm" color="gray.500">Tx: {formattedHash}</Text>
      ...
      {(fromChain || toChain) && (
        <Flex align="center" mb={4}>
          {fromChain && (
            <HStack>
              {fromChain.icon || <Box boxSize={4} borderRadius="full" bg={fromChain.color || 'gray.400'} />}
          ...
          {toChain && (
            <HStack>
              {toChain.icon || <Box boxSize={4} borderRadius="full" bg={toChain.color || 'gray.400'} />}
...
// Notification Card
const NotificationCard: React.FC<{ notification: NotificationInfo }> = ({ notification }) => {
  const { 
    type, 
    title, 
    message, 
    ...
    <Box opacity={isRead ? 0.7 : 1}>
      <Flex align="flex-start" mb={2}>
        ...
        <Box flex="1">
          ...
          <Text fontSize="sm" mb={action ? 3 : 0}>{message}</Text>
...
export const EnhancedCard: React.FC<EnhancedCardProps> = ({
  ...
  // Hooks
  const { isOpen: isExpanded, onToggle } = useDisclosure({ defaultIsOpen: isInitiallyExpanded });
  ...
  return (
    <MotionBox
      ...
      backdropFilter={glassMorphism ? `blur(${glassMorphismStrength}px)` : undefined}
      {...rest}
    >
      {/* Drag Handle */}
      {isDraggable && (
      ...
      {/* Card Header */}
      {(title || subtitle || avatar || headerIcon || headerActions || isCollapsible) && (
        <Flex
          align="center"
        ...
        >
          <Flex align="center">
            ...
            <Box>
              {title && <Heading size={size === 'sm' ? 'xs' : 'md'}>{title}</Heading>}
          ...
          <HStack spacing={2}>
            {headerActions && headerActions.map((action, index) => (
              <Tooltip key={index} label={action.tooltip} isDisabled={!action.tooltip}>
                <IconButton
      ...
      {title && subtitle && (
        <Box px={getCardPadding()} pb={3} pt={0} borderBottomWidth={1} borderColor={borderColor}>
