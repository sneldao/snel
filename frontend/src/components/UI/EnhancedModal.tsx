...
import { motion, AnimatePresence, useAnimation, Variants, PanInfo, useDragControls } from 'framer-motion';
...
const slideVariants: Variants = {
  ...
  exit: (placement: ModalPlacement) => {
    switch (placement) {
      case 'top': return { y: '-100%', opacity: 0, transition: { duration: 0.2 } };
      case 'right': return { x: '100%', opacity: 0, transition: { duration: 0.2 } };
      case 'bottom': return { y: '100%', opacity: 0, transition: { duration: 0.2 } };
      case 'left': return { x: '-100%', opacity: 0, transition: { duration: 0.2 } };
      default: return { y: 20, opacity: 0, transition: { duration: 0.2 } };
    }
  },
};
...
const drawerVariants: Variants = {
  ...
  exit: (placement: ModalPlacement) => {
    switch (placement) {
      case 'top': return { y: '-100%', transition: { duration: 0.3 } };
      case 'right': return { x: '100%', transition: { duration: 0.3 } };
      case 'bottom': return { y: '100%', transition: { duration: 0.3 } };
      case 'left': return { x: '-100%', transition: { duration: 0.3 } };
      default: return { x: '100%', transition: { duration: 0.3 } };
    }
  },
};
...
const EnhancedBackdrop: React.FC<{
  ...
  const bgColor = overlayColor || useColorModeValue('blackAlpha.600', 'blackAlpha.700');
  ...
  return (
    <Box mb={4}>
      <Flex
        ...
        _hover={section.isCollapsible ? { bg: useColorModeValue('gray.100', 'gray.700') } : {}}
      >
        <Flex align="center">
    ...
    <VStack spacing={4} align="stretch">
      {wallets.map((wallet) => (
        <Flex
          ...
          bg={useColorModeValue('white', 'gray.800')}
          _hover={wallet.onClick ? { bg: useColorModeValue('gray.50', 'gray.700') } : {}}
          opacity={!wallet.isInstalled ? 0.6 : 1}
        >
          <Flex align="center">
            <Box mr={4} fontSize="2xl">
              {typeof wallet.icon === 'string' ? (
                <img src={wallet.icon as string} alt={wallet.name} width="40px" height="40px" />
              ) : (
        ...
        section={{
          ...
          content: (
            <VStack spacing={3} align="stretch">
              ...
              {data && (
                <Box>
                  <Text color="gray.500" mb={1}>Data</Text>
                  <Box
                    p={2}
                    borderWidth="1px"
                    borderRadius="md"
...
const ConfirmationContent: React.FC<{
  ...
  const getColor = () => {
    switch (type) {
      case 'info': return useColorModeValue('blue.500', 'blue.300');
      case 'success': return useColorModeValue('green.500', 'green.300');
      case 'warning': return useColorModeValue('orange.500', 'orange.300');
      case 'error': return useColorModeValue('red.500', 'red.300');
      case 'question': return useColorModeValue('purple.500', 'purple.300');
      default: return useColorModeValue('blue.500', 'blue.300');
    }
  };
...
export const EnhancedModal = forwardRef<HTMLDivElement, EnhancedModalProps>((props, ref) => {
  const {
    // Base props
    isOpen,
    onClose,
  ...
  const getModalSize = () => {
    ...
    if (variant === 'drawer') {
      switch (size) {
        case 'xs': return { width: '280px', height: '100%' };
        case 'sm': return { width: '320px', height: '100%' };
        case 'md': return { width: '380px', height: '100%' };
        case 'lg': return { width: '440px', height: '100%' };
        case 'xl': return { width: '520px', height: '100%' };
        case 'full': return { width: '100%', height: '100%' };
        default: return { width: '380px', height: '100%' };
      }
    }
    
    if (variant === 'sheet') {
      switch (size) {
        case 'xs': return { width: '100%', height: '20%' };
        case 'sm': return { width: '100%', height: '30%' };
        case 'md': return { width: '100%', height: '50%' };
        case 'lg': return { width: '100%', height: '70%' };
        case 'xl': return { width: '100%', height: '85%' };
        case 'full': return { width: '100%', height: '100%' };
    ...
    switch (size) {
      case 'xs': return { width: '320px', maxWidth: '95vw' };
      case 'sm': return { width: '384px', maxWidth: '95vw' };
      case 'md': return { width: '512px', maxWidth: '95vw' };
      case 'lg': return { width: '720px', maxWidth: '95vw' };
      case 'xl': return { width: '920px', maxWidth: '95vw' };
      case 'full': return { width: '100vw', height: '100vh', maxWidth: '100vw' };
      default: return { width: '512px', maxWidth: '95vw' };
    }
  };
  ...
  if (variant === 'drawer') {
    const drawerPlacement = getModalPlacement() as 'top' | 'right' | 'bottom' | 'left';
              ...
              >
                ...
                {showHeader && (title || subtitle) && (
                  ...
                  >
                    ...
                    <Box flex="1">
                      {title && (
                        <Heading size="md" id={`drawer-${id || 'modal'}-title`}>
                          {title}
                        </Heading>
                      )}
                      {subtitle && (
                    ...
                    {headerActions && (
                      <HStack spacing={2}>
                        {headerActions.map((action, index) => (
                          <Tooltip key={index} label={action.tooltip || action.label}>
                            <IconButton
                ...
                {showFooter && (primaryAction || secondaryAction || additionalActions) && (
                  <Flex
                    p={6}
                    borderTopWidth={1}
                    borderColor={useColorModeValue('gray.200', 'gray.700')}
                    ...
                    <HStack spacing={3}>
                      {secondaryAction && (
                        <Button
                          ...
                          colorScheme={secondaryAction.colorScheme || 'gray'}
                          onClick={() => {
                          ...
                          leftIcon={secondaryAction.icon ? <>{secondaryAction.icon}</> : undefined}
                        >
                      ...
                      {primaryAction && (
                        <Button
                          ...
                          leftIcon={primaryAction.icon ? <>{primaryAction.icon}</> : undefined}
                        >
                          {primaryAction.label}
                ...
                <VisuallyHidden>
                  <div aria-live="polite">
                    {modalState === 'loading' ? 'Loading...' : 
                     modalState === 'success' ? 'Operation completed successfully' : 
                     modalState === 'error' ? 'An error occurred' : ''}
                  </div>
                </VisuallyHidden>
              </MotionDrawerContent>
              ...
              >
                ...
                {showHeader && (title || subtitle) && (
                  ...
                  >
                    ...
                    <Box flex="1">
                      {title && (
                        <Heading size="md" id={`sheet-${id || 'modal'}-title`}>
                          {title}
                        </Heading>
                      )}
                      {subtitle && (
              ...
              <FocusScope contain restoreFocus autoFocus>
                ...
                >
                  <MotionBox
                    ...
                    style={isDraggable ? { x: position.x, y: position.y } : undefined}
                    aria-modal="true"
                  ...
                  >
                    ...
                    {showHeader && (title || subtitle) && (
                      ...
                      >
                        ...
                        <Box flex="1">
                          {title && (
                            <Heading size="md" id={`modal-${id || 'modal'}-title`}>
                              {title}
                            </Heading>
                          )}
                          {subtitle && (
