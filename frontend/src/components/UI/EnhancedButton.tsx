...
export interface EnhancedButtonProps extends Omit<ChakraButtonProps, 'variant' | 'size'> {
...
export const EnhancedButton = forwardRef<EnhancedButtonProps, 'button'>((props, ref) => {
  const {
    variant = 'primary',
    size = 'md',
    state = 'idle',
  ...
  const getButtonBorder = () => {
    if (variant === 'outline') {
      switch (variant) {
        ...
        case 'danger':
          return `1px solid ${dangerColor}`;
        case 'success':
          return `1px solid ${successColor}`;
        case 'wallet':
          return `1px solid ${walletColor}`;
        case 'transaction':
          return `1px solid ${transactionColor}`;
        default:
    ...
    return (
      ...
      >
        ...
        <AnimatePresence>
          <MotionBox
            textAlign="center"
            variants={state !== 'idle' ? { visible: { opacity: 1 } } : undefined}
          ...
          <AnimatePresence>
            ...
            >
              {state === 'loading' ? (
                ...
                showEndIcon && React.cloneElement(endIcon as React.ReactElement, {
                  size: getIconSize(),
                })
              )}
            </MotionBox>
        ...
        {state === 'progress' && (
          ...
          >
            <Progress
              value={progress}
              size="xs"
              colorScheme={variant === 'primary' ? 'blue' : 
                variant === 'danger' ? 'red' : 
                variant === 'success' ? 'green' : 
                variant === 'wallet' ? 'purple' : 
                variant === 'transaction' ? 'orange' : 'blue'}
              hasStripe
              isAnimated
            />
          </Box>
  ...
  if (variant === 'transaction') {
    return (
      <Tooltip label={tooltipText || transactionHash} isDisabled={!tooltipText && !transactionHash}>
        <MotionChakraButton
          ref={ref}
          display="flex"
          alignItems="center"
          ...
          {...getButtonPadding()}
          fontSize={getButtonFontSize()}
          isDisabled={isDisabled || state === 'loading' || state === 'confirming'}
          onClick={handleClick}
          _hover={!isDisabled ? getButtonHoverStyle() : undefined}
          _active={!isDisabled ? { transform: 'scale(0.98)' } : undefined}
          position="relative"
    ...
    <Tooltip label={tooltipText} isDisabled={!tooltipText}>
      ...
      >
        <MotionChakraButton
          ...
          variants={!disableAnimation ? buttonAnimationVariants : undefined}
          initial="idle"
          animate={controls}
