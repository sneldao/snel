...
const TokenAmountInput = forwardRef<HTMLInputElement, EnhancedInputProps>(
  (props, ref) => {
    const {
      token,
      showMaxButton,
                ...
                >
                  {token.icon && (
                    <Box
                      mr={1}
                      boxSize={
                        size === "sm" ? "14px" : size === "lg" ? "24px" : "18px"
                      }
                      position="relative"
                    >
                      <Image
...
const AddressInput = forwardRef<HTMLInputElement, EnhancedInputProps>(
  (props, ref) => {
    const {
      value = "",
      onChange,
    ...
    return (
      <InputGroup size={size}>
        <InputLeftElement>
          <Icon
            as={FaWallet}
            color={isValidAddress ? "green.500" : "gray.400"}
          />
        </InputLeftElement>
        <Input
          ref={ref}
...
const SearchInput = forwardRef<HTMLInputElement, EnhancedInputProps>(
  (props, ref) => {
    const {
      value = "",
      onChange,
...
const PasswordInput = forwardRef<HTMLInputElement, EnhancedInputProps>(
  (props, ref) => {
    const {
      value = "",
      onChange,
...
const CurrencyInput = forwardRef<HTMLInputElement, EnhancedInputProps>(
  (props, ref) => {
    const {
      value = "",
      onChange,
...
const PercentageInput = forwardRef<HTMLInputElement, EnhancedInputProps>(
  (props, ref) => {
    const {
      value = "",
      onChange,
...
export const EnhancedInput = forwardRef<HTMLInputElement, EnhancedInputProps>(
  (props, ref) => {
    ...
    // State
    const [localValidationState, setLocalValidationState] =
      useState<ValidationState>(validationState);
    ...
    // Hooks
    const { hasCopied, onCopy: copyToClipboard } = useClipboard(
    ...
    const validateValue = React.useCallback(
      async (valueToValidate: any): Promise<ValidationResult> => {
        ...
        if (isRequired && (!valueToValidate || valueToValidate === "")) {
          const result = {
            ...
            type: "invalid" as ValidationState,
          };
        ...
        if (asyncValidator) {
          ...
          try {
            ...
            setLocalValidationState(
              result.isValid ? "valid" : result.type || "invalid"
            );
    ...
    const renderSpecializedInput = () => {
      ...
      switch (inputType) {
        ...
        case "percentage":
          return <PercentageInput {...commonProps} precision={precision} />;
        default:
          return null;
      }
    };
        ...
        <Box position="relative" width={isFullWidth ? "100%" : "auto"}>
          ...
          {label && (
            <MotionBox
              ...
              left="10px"
              transform={
                shouldFloatLabel ? "translateY(0)" : "translateY(-50%)"
              }
              px={1}
              bg={bgColor}
              zIndex={1}
            ...
            >
              <Text
                ...
                color={
                  shouldFloatLabel ? focusedLabelColor : floatingLabelColor
                }
              >
                {label}
                {isRequired && (
                  <Box as="span" color={errorColor} ml={1}>
                    *
                  </Box>
                )}
              </Text>
          ...
          {(helperText || getValidationMessage()) && (
            ...
            >
              {localValidationState !== "none" ? (
                <Text fontSize="sm" color={getValidationColor()} mt={1}>
