...
const StatusIcon: React.FC<{ status: string; size?: number }> = ({ status, size = 5 }) => {
  switch (status) {
    case 'completed':
    ...
    case 'failed':
      return <Icon as={FaExclamationTriangle} color="red.500" w={size} h={size} />;
    default:
      return <Icon as={FaClock} color="gray.400" w={size} h={size} />;
  }
};

const StepProgressBar: React.FC<{ steps: TransactionStep[]; currentStep: number }> = ({ steps, currentStep }) => {
  ...
  return (
    <Box w="100%" mb={4}>
      ...
      <Box position="relative" h="8px" w="100%" bg={bgColor} borderRadius="full" overflow="hidden">
        <MotionProgress
          value={progress}
          size="sm"
...
const RouteVisualization: React.FC<{ route: RouteNode[]; activeNodeIndex: number }> = ({ route, activeNodeIndex }) => {
  ...
  return (
    <Box 
      ...
      mb={4}
    >
      <Text fontSize="sm" fontWeight="medium" mb={3}>Transaction Route</Text>
...
const DetailedSteps: React.FC<{ steps: TransactionStep[] }> = ({ steps }) => {
  ...
  return (
    <VStack spacing={3} align="stretch" w="100%">
      <Text fontSize="sm" fontWeight="medium" mb={1}>Transaction Steps</Text>
      <AnimatePresence>
          ...
          >
            ...
            >
              ...
              <VStack align="start" ml={3} spacing={0} flex={1}>
                <Text fontSize="sm" fontWeight="semibold">{step.title}</Text>
                ...
                {step.estimatedTime && step.status !== 'completed' && (
                  <HStack mt={1} spacing={1}>
                    ...
                    <Text fontSize="xs" color="gray.500">Est. {step.estimatedTime}</Text>
                  </HStack>
                )}
              ...
              {step.txHash && (
                <Tooltip label="View in explorer">
                  <Link 
                    ...
                    isExternal
                  >
                    <Icon as={FaExternalLinkAlt} color="gray.400" _hover={{ color: 'blue.500' }} />
                  </Link>
                </Tooltip>
              )}
            </Flex>
...
const FeeBreakdownDisplay: React.FC<{ 
  ...
  const totalPercentage = parseFloat(feeBreakdown.totalFee) / 
  ...
  return (
    ...
    >
      <Flex justify="space-between" align="center" mb={3}>
        <Text fontSize="sm" fontWeight="medium">Fee Breakdown</Text>
      ...
      <SimpleGrid columns={1} spacing={2} mb={3}>
        ...
        <HStack justify="space-between">
          <HStack>
            ...
            <Text fontSize="sm" fontWeight="bold">Total Fee</Text>
          </HStack>
              ...
              <VStack align="start" spacing={2}>
                {optimizationTips.map((tip, index) => (
                  <HStack key={index} align="start" spacing={2}>
                    <Icon as={FaInfoCircle} color="blue.500" mt="3px" fontSize="xs" />
              ...
              <Alert status="warning" mb={4} borderRadius="md">
                ...
                <Box>
                  ...
                  <AlertDescription fontSize="sm">
                    Transaction {txHash.substring(0, 6)}...{txHash.substring(txHash.length - 4)} is stuck or failed.
              ...
              <VStack spacing={3} align="stretch">
                {options.map((option) => (
                  <Box 
                    ...
                    borderLeftWidth="4px"
                    borderLeftColor={
                      option.severity === 'high' ? 'red.500' :
                      option.severity === 'medium' ? 'orange.500' : 'yellow.500'
                    }
                  ...
                  >
                    <Flex justify="space-between" align="center">
                      ...
                      <Button 
                        size="sm"
                        colorScheme={
                          option.severity === 'high' ? 'red' :
                          option.severity === 'medium' ? 'orange' : 'yellow'
                        }
                        isLoading={selectedOption === option.id && actionLoading}
                      >
                        {option.buttonText}
                      </Button>
                    </Flex>
...
export const CrosschainTransactionTracker: React.FC<CrosschainTransactionTrackerProps> = ({
  txHash,
  sourceChain,
  destinationChain,
  asset,
  ...
  }, [txHash, sourceChain, destinationChain, sourceChainName, destinationChainName, asset, amount]);
      ...
      try {
        ...
        const status = await axelarServiceV2.getTransactionStatus(
          ...
          typeof sourceChain === 'number' 
            ? ChainUtils.getAxelarChainName(sourceChain) 
            : sourceChain,
          typeof destinationChain === 'number'
            ? ChainUtils.getAxelarChainName(destinationChain)
            : destinationChain
        );
  ...
  }, [txHash, sourceChain, destinationChain, isPolling, sourceChainName, destinationChainName, sourceAddress, destinationAddress, asset, amount]);
  ...
  }, [steps, currentStepIndex, routeNodes, activeRouteNodeIndex, onComplete, onRecoveryNeeded, txHash]);
      ...
      const options: RecoveryOption[] = [
        {
          ...
          description: 'Attempt to retry the transaction with the same parameters',
          action: async () => {
            ...
            const updatedSteps = steps.map(step => ({
              ...step,
              status: step.id === 'initiate' ? 'completed' : 
                      step.id === 'source_confirm' ? 'active' : 'pending',
          ...
          action: async () => {
            ...
            const updatedSteps = steps.map(step => ({
              ...
              status: step.id === 'initiate' ? 'completed' : 
                      step.id === 'source_confirm' ? 'active' : 'pending',
              timestamp: step.id === 'initiate' ? Date.now() : undefined,
              description: step.id === 'axelar_processing' ? 
                'Validating and preparing cross-chain message (Accelerated)' : step.description
            }));
  ...
  }, [steps, routeNodes, sourceChainName, amount, asset, sourceChain, closeRecoveryModal, openRecoveryModal]);
      ...
      >
        <Flex justify="space-between" align="center">
          ...
          <Badge 
            colorScheme={
              error ? 'red' : 
              currentStepIndex >= steps.length - 1 ? 'green' : 'blue'
            }
            fontSize="sm"
            py={1}
            px={2}
          ...
          >
            {error ? 'Failed' : 
             currentStepIndex >= steps.length - 1 ? 'Completed' : 'In Progress'}
          </Badge>
        </Flex>
      </Box>
      ...
      <Box p={4}>
        ...
        <SimpleGrid columns={{ base: 1, md: 2 }} spacing={4} mb={4}>
          <Stat>
            ...
            <StatNumber>{amount} {asset}</StatNumber>
            <StatHelpText>
          ...
          <Stat>
            ...
            <StatNumber>
              {txDetails?.estimatedCompletionTime ? (
                new Date(txDetails.estimatedCompletionTime).toLocaleTimeString()
              ) : (
            ...
            <StatHelpText>
              {currentStepIndex === steps.length - 1 ? 
                'Completed' : `Step ${currentStepIndex + 1} of ${steps.length}`}
            </StatHelpText>
          </Stat>
        </SimpleGrid>
        ...
        {/* Route visualization */}
        <RouteVisualization route={routeNodes} activeNodeIndex={activeRouteNodeIndex} />
