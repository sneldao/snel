...
from ...services.error_guidance_service import ErrorContext
...
class TransferProcessor(BaseProcessor):
    ...
    async def process(self, unified_command: UnifiedCommand) -> UnifiedResponse:
        ...
        try:
            ...
            wallet_address: str = unified_command.wallet_address
            chain_id: int = unified_command.chain_id or 1
            ...
            if (
                details
                and details.token_in
                and details.token_in.symbol.upper() == "MNEE"
                and chain_id != 1
            ):
                logger.info(
                    f"MNEE specified on chain {chain_id}, redirecting to Ethereum mainnet (chain 1) for X402 protocol"
                )
                # Update the local chain_id to use Ethereum
                chain_id = 1
            ...
            logger.info(
                f"DEBUG transfer: raw_amount={details.amount}, formatted={amount_str}, token={details.token_in.symbol}, dest={details.destination}"
            )
            ...
            token_address = full_token.get_address(chain_id)
            if not token_address:
                ...
                common_chain_ids = [
                    ...
                    137,
                    25,
                ]  # Ethereum, Base, Arbitrum, Optimism, Polygon, Cronos
                for cid in common_chain_ids:
                    if full_token.is_supported_on_chain(cid):
                        supported_chains.append(self._get_chain_name(cid))
            ...
            # Estimate gas costs
            gas_estimate: dict[str, Any] = token_query_service.estimate_gas(
                chain_id, transaction_type="erc20_transfer"
            )

            # Check if this transfer could benefit from batching
            gas_optimization_hint = self._check_gas_optimization_opportunity(
                unified_command
            )
            ...
            transaction_data = self._create_transaction_data(result, chain_id)

            # DEBUG: Log response before sending
            logger.info(
                f"DEBUG response amount_str={amount_str}, token={details.token_in.symbol}, full_token.decimals={full_token.decimals}"
            )
            ...
            return self._create_error_response(
                f"Transfer failed: {str(e)}", AgentType.TRANSFER, str(e)
            )

    def _check_gas_optimization_opportunity(
        ...
        if chain_id in [
            ...
            324,
            25,
        ]:  # Scroll, Arbitrum, Optimism, Base, zkSync, Cronos
    ...
    def _get_chain_name(self, chain_id: int) -> str:
        """
        Get chain name from chain ID using centralized registry.
        ...
        56: "BSC"
        """
        from ...config.chains import get_chain_name
