...
from ..core.parser.unified_parser import unified_parser
...
class CommandProcessor:
    ...
    def __init__(
        ...
        from ..services.axelar_gmp_service import axelar_gmp_service

        self.gmp_service = axelar_gmp_service

        # Import enhanced cross-chain handler for GMP operations
        from ..services.enhanced_crosschain_handler import enhanced_crosschain_handler
        ...
        self.processor_registry = ProcessorRegistry(
            ...
            transaction_flow_service=self.transaction_flow_service,
        )
...
- GREETING: Simple greetings like "hi", "hello", "hey", "gm", "gn", "gnight", "good morning", "good night", "sup", "yo", "howdy"
- CONFIRMATION: Yes/no confirmations
- UNKNOWN: Unclear or unrelated commands

Classification guidelines:
- GREETING examples: "gm", "good morning", "hello", "hi", "hey", "sup", "yo", "gn", "gnight" → GREETING
...
"""
            ...
            response = await client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a command classifier. Respond with only the command type name.",
                    },
            ...
            ai_classification = content.strip().upper() if content else "UNKNOWN"
        ...
        try:
            ...
            # Step 1: Classification if UNKNOWN
                ...
                if ai_type != CommandType.UNKNOWN:
                    ...
                    logger.info(f"Command classified by AI as: {ai_type.value}")
                else:
                    ...
                    logger.info(
                        "AI classification unavailable or failed, defaulting to CONTEXTUAL_QUESTION"
                    )
            ...
            if unified_command.command_type in [
                ...
                CommandType.BRIDGE,
                CommandType.SWAP,
                CommandType.CROSS_CHAIN_SWAP,
                CommandType.GMP_OPERATION,
            ...
            if (
                ...
                and self.settings
                and unified_command.chain_id
                not in self.settings.chains.supported_chains
            ):
            ...
            logger.exception(
                f"Unexpected error processing command: {unified_command.command}"
            )
            return UnifiedResponse(
                content={
    ...
    async def _should_use_gmp_handler(self, unified_command: UnifiedCommand) -> bool:
        ...
        try:
            if unified_command.command_type == CommandType.CROSS_CHAIN_SWAP:
                return True
            return False
        except Exception as e:
    ...
    async def _process_gmp_operation(
        ...
        try:
            ...
            return await self.gmp_handler.handle_operation(unified_command)
        except Exception as e:
            logger.exception(f"Error in GMP processing: {e}")
    ...
    async def _process_transaction_step_complete(
        self, unified_command: UnifiedCommand
    ) -> UnifiedResponse:
        """
        ...
        try:
            ...
            if hasattr(step_data, "model_dump"):
                ...
                logger.info(f"Transaction step complete - model_dump: {data_dict}")
            elif hasattr(step_data, "__dict__"):
                data_dict = step_data.__dict__
                logger.info(f"Transaction step complete - __dict__: {data_dict}")
            else:
                data_dict = {}

            wallet_address = (
                data_dict.get("wallet_address") or unified_command.wallet_address
            )
            chain_id = data_dict.get("chain_id") or unified_command.chain_id
            tx_hash = data_dict.get("tx_hash")
            success = data_dict.get("success", True)
            error = data_dict.get("error")

            if not wallet_address or not tx_hash:
                raise ValidationError(
                    "Missing required fields: wallet_address and tx_hash"
                )

            logger.info(
                f"Completing transaction step for {wallet_address}, tx: {tx_hash}"
            )
            ...
            if next_step:
                ...
                current_flow = self.transaction_flow_service.get_current_flow(
                    wallet_address
                )
                ...
                return UnifiedResponse(
                    content={
                        ...
                        "flow_info": {
                            ...
                            "total_steps": (
                                len(current_flow.steps) if current_flow else 1
                            ),
                        },
                    },
                    agent_type=AgentType.DEFAULT,
    ...
    async def _process_unknown(
        ...
        """
        from ..services.error_guidance_service import (
            ErrorContext,
            error_guidance_service,
        )
        ...
        if any(
            word in command_lower for word in ["bridge", "private", "zcash", "privacy"]
        ):
            guidance = error_guidance_service.get_guidance(
                CommandType.BRIDGE, ErrorContext.MISSING_AMOUNT
            )
            return UnifiedResponse(
                content={
                    "message": "I can help with bridges and privacy! "
                    + guidance["message"],
                    "type": "help",
        ...
        if any(word in command_lower for word in ["swap", "exchange", "trade"]):
            guidance = error_guidance_service.get_guidance(
                CommandType.SWAP, ErrorContext.MISSING_TOKEN_PAIR
            )
            return UnifiedResponse(
                content={
        ...
        if any(word in command_lower for word in ["transfer", "send", "move"]):
            guidance = error_guidance_service.get_guidance(
                CommandType.TRANSFER, ErrorContext.MISSING_DESTINATION
            )
            return UnifiedResponse(
                content={
        ...
        guidance = error_guidance_service.get_guidance(
            CommandType.UNKNOWN, ErrorContext.RESEARCH_HINT
        )
        return UnifiedResponse(
            content={
                "message": "I'm not exactly sure how to handle that request. "
                + guidance["message"],
                "type": "help",
                "suggestions": guidance["suggestions"]
                + [
